...
private:
    ...
    TQue<QuePosition::VECIN, 2> stage0InQue;
    TQue<QuePosition::VECOUT, 2> stage0OutQue;
    TBuf<> stage2OutBuf;
...
{
    constexpr uint32_t softmaxBufSize = 256;
...
{
    LocalTensor<T> sinksUb = this->sinksBuf.template Get<T>();
    const uint32_t maxN = constInfo.gSize;
    DataCopyExtParams dataCopyParams;
...
{
    // ub buffer
    pipe->InitBuffer(dequantScaleBuff, 64 * 16 * 2 * sizeof(float));
    ...
    tPipe->InitBuffer(stage0InQue, 2, dVTemplateTypeInput * 16 * sizeof(KV_T));
    tPipe->InitBuffer(stage0OutQue, 2, dVTemplateType * (16 + 1) * sizeof(Q_T));
