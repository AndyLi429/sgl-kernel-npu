    ...
    const Vec1SliceInfo &sliceInfo, uint32_t dStartIdx, uint32_t dDealSize, uint32_t dBaseSize, uint32_t stateIdx)
{
    ...
    if constexpr (COMP::cacheMode == CACHE_MODE::RING_BUFFER) {
        ...
        srcBaseOffset += (max(startSeqIdx, (uint64_t)writeSeqStartIdx) - startSeqIdx) * coff_ * dBaseSize;
    ...
    if constexpr (COMP::xLayout == X_LAYOUT::BSH) {
        ...
        while (compressTcSize > 0) {
            ...
            if (preScSize < totalScSize) {
                ...
                // ubOffset：slice 跨 batch 时（while 多次迭代）必须从 scoreLocal[ubOffset]
                // 拷贝（PadAlign 按压缩块连续排布，每块 coff*cmpRatio 行），否则会把
                // 第一个 slice 的内容重复输出（同 NORMAL 模板的修复）。
    ...
    uint32_t dBaseOffset, uint32_t dDealSize, uint32_t dBaseSize, uint32_t dealSeqStartIdx)
{
    ...
    if (statisticInfo.compressorScCnt > 0) {
        ...
        if constexpr (COMP::gradEnabled == GRAD_ENABLED::ENABLE) {
            CopyOutMidResToOutput(kvLocal, scoreLocal, originSliceInfo, statisticInfo.compressorScCnt,
        ...
        PipeBarrier<PIPE_V>();
        CopyOutVec1ResToOutput(comperssoredUb, originSliceInfo, statisticInfo.compressorScCnt, dStartIdx + dBaseOffset,
                               dDealSize);
    }
